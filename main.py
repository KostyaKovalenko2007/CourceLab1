...
class VK2YaDiskBackup:
    ...
    def get_vk_profile_photos_list(self):
        url = f"https://api.vk.com/method/photos.get?owner_id={self.owner}" \
              f"&album_id=profile&extended=1&access_token={self.vk_token}&v=5.131"
    ...
    def yadisk_upload_file(self,image:dict):
        folder_resp = self.session.put(f'{self.base_url}/resources?path={self.owner}_img')
        if folder_resp.status_code == 409 or folder_resp.status_code == 201:
            file_url = self.session.get(f'{self.base_url}/resources/upload?path={self.owner}_img/{str(image["likes"])}.jpg&overwrite=false')
            yadisk_file_name = f'{str(image["likes"])}.jpg'
            if file_url.status_code == 409:
                file_url = self.session.get(
                    f'{self.base_url}/resources/upload?path={self.owner}_img/{str(image["likes"])}_'\
